...
class siamese:
    ...
    def fc_layer(self, bottom, n_weight, name):
        ...
        W = tf.get_variable('W', dtype=tf.float32, shape=[n_prev_weight, n_weight], initializer=initer)
        b = tf.get_variable('b', dtype=tf.float32, initializer=tf.constant(0.01, shape=[n_weight]))
    ...
    def loss_custom(self):
        ...
        diff_var = tf.sub(tf.pow(tf.sub(self.o1, self.o2), 2),tf.pow(diff_mean,2))
