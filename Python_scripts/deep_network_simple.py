...
def train_neural_network(xx1,xx2,xx1_test,xx2_test,jj):
    model = siamese()
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            while i < len(xx1):
                ...
                _, c, p1, p2 = sess.run([optimizer, model.loss, model.o1, model.o2], feed_dict={model.x1: batch_x1,model.x2: batch_x2, model.dropout_f: 0.7})
                epoch_loss += c
                O1 = np.append(O1, p1)
                O2 = np.append(O2, p2)
            print('Ens ',jj,' Epoch', epoch+1, '/',hm_epochs,'Loss:',epoch_loss, c)
...
O1_t=np.empty([len(X1_test),hm_ensemble])
O2_t=np.empty([len(X1_test),hm_ensemble])
for ii in range(hm_ensemble):                 
    [Ox1, Ox2, Ox1_t, Ox2_t] = train_neural_network(X1_train, X2_train, X1_test, X2_test,ii)
    O1_t[:,ii]=np.squeeze(Ox1_t)
    O2_t[:,ii]=np.squeeze(Ox2_t)
O1_t = np.mean(O1_t,axis=1)
O2_t = np.mean(O2_t,axis=1)
...
#fim2 = np.squeeze(np.asarray(np.sum(all_X2[:,:num_col]-all_X1[:,:num_col],axis=1)))
